    ...
    for report in tqdm(processed_reports[:]):
        ...
        if 4 in N_GRAM_SIZES:
            for i in range(0, end - 3):
                if (report[i + 1] != "no" and report[i + 2] != "no" and report[i + 3] != "no" and report[
                    i].lower() != "sentenceend" and report[i + 1].lower() != "sentenceend" and report[
                    i + 2].lower() != "sentenceend" and report[i + 3] != "sentenceend"):  # no only at beginning
                    new_report.append(report[i] + "_" + report[i + 1] + "_" + report[i + 2] + "_" + report[i + 3])
            # CREATES 3-grams
        if 3 in N_GRAM_SIZES:
            for i in range(0, end - 2):
                if (report[i + 1] != "no" and report[i + 2] != "no" and report[i].lower() != "sentenceend" and report[
                    i + 1].lower() != "sentenceend" and report[i + 2].lower() != "sentenceend"):  # no only at beginning
                    new_report.append(report[i] + "_" + report[i + 1] + "_" + report[i + 2])
        # CREATES 2-grams
        if 2 in N_GRAM_SIZES:
            for i in range(0, end - 1):
                if (report[i + 1] != "no" and report[i].lower() != "sentenceend" and report[
                    i + 1].lower() != "sentenceend"):  # no only at beginning
...
def build_train_test_corpus(df_data, ngram_list, labeled_filepath, TRAIN_INDEX_OVERRIDE):
    ...
    for i in range(0, df_data.shape[0]):
        if df_data['Accession Number'].iloc[i] in labeled_cases and df_data['Accession Number'].iloc[
            i] not in TRAIN_INDEX_OVERRIDE:
...
def build_d2v_corpora(df_data, d2v_inputs, labeled_indices):
    ...
    print('%d unlabeled reports for featurization, %d labeled reports for modeling' % (
    len(unlabeled_docs), len(labeled_docs)))
...
def train_d2v(unlabeled_docs, labeled_docs, D2V_EPOCH, DIM_DOC2VEC, W2V_DM, W2V_WINDOW, total_unlabeled_words):
    ...
    d2vmodel = gensim.models.Doc2Vec(dm=W2V_DM, size=DIM_DOC2VEC, window=W2V_WINDOW, negative=5, hs=0, min_count=2,
                                     workers=cores)
...
def calc_auc(predictor_matrix, eligible_outcomes_aligned, all_outcomes_aligned, N_LABELS, pred_type, header,
    ...
    confusion = pd.DataFrame(
        data=np.zeros(shape=(eligible_outcomes_aligned.shape[1] * len(model_types), 6), dtype=np.int),
        columns=['Label (with calcs on held out 40 pct)', 'AUC', 'True +', 'False +', 'True -', 'False -'])
    ...
    for i in range(0, N_LABELS):
        ...
        if (len(set(eligible_outcomes_aligned.ix[train, i].tolist()))) == 1:
            PROCEED = False;
            raise ValueError(
                "fed label to lasso regression with no variation - cannot compute - please investigate data")
            ...
            for model_type in model_types:
                if (model_type == "Lasso"):
                    ...
                    try:
                        ...
                        grid_search = GridSearchCV(LogisticRegression(), param_grid=parameters, scoring='neg_log_loss',
                                                   cv=cv)
                        ...
                        raise ValueError(
                            "error in lasso regression - likely data issue, may involve rare labels - please investigate data")
                ...
                df0 = pd.DataFrame({'predict': pred0, 'target': eligible_outcomes_aligned.ix[test, i],
                                    'label': all_outcomes_aligned['Accession Number'][test]})
...
def generate_labeled_data_features(labeled_file,
    ...
    for i in tqdm(labeled_indices):
        if df_data['Accession Number'].iloc[i] not in list(outcomes['Accession Number']):
            raise Exception(" df_data i @ " + str(i) + " = " + str(
                df_data['Accession Number'].iloc[i]) + " not in set of held out cases, examine")
...
def write_silver_standard_labels(corpus,
    ...
    for x in range(0, len(corpus), 2000):
        ...
        if (DO_BOW and DO_WORD2VEC and DO_PARAGRAPH_VECTOR): whole_combined_matrix = np.hstack(
            (whole_bow_matrix, whole_w2v_matrix, whole_pv_matrix))

        if (DO_BOW and DO_WORD2VEC and not DO_PARAGRAPH_VECTOR): whole_combined_matrix = np.hstack(
            (whole_bow_matrix, whole_w2v_matrix))
        if (DO_BOW and not DO_WORD2VEC and DO_PARAGRAPH_VECTOR): whole_combined_matrix = np.hstack(
            (whole_bow_matrix, whole_pv_matrix))
        if (not DO_BOW and DO_WORD2VEC and DO_PARAGRAPH_VECTOR): whole_combined_matrix = np.hstack(
            (whole_w2v_matrix, whole_pv_matrix))
    ...
    pred_outcome_proba_df = pd.DataFrame(pred_outcome_matrix_proba, index=accession_list,
                                         columns=list(eligible_outcomes_aligned.columns.values))
    pred_outcome_binary_df = pd.DataFrame(pred_outcome_matrix_binary, index=accession_list,
                                          columns=list(eligible_outcomes_aligned.columns.values))
...
class RadReportAnnotator(object):
    ...
    def define_config(self, DO_BOW=True, DO_WORD2VEC=False, DO_PARAGRAPH_VECTOR=False, DO_SILVER_STANDARD=True,
                      STEM_WORDS=True, N_GRAM_SIZES=[1], DIM_DOC2VEC=200, N_THRESH_CORPUS=1, N_THRESH_OUTCOMES=1,
                      TRAIN_INDEX_OVERRIDE=[], SILVER_THRESHOLD="mean", NAME_UNID_REPORTS="Accession Number",
                      NAME_TEXT_REPORTS="Report Text"):
        """
        ...
        if not N_GRAM_SIZES in ([1], [2], [3], [1, 2], [1, 3], [1, 2, 3]):
            raise ValueError('Invalid N_GRAM_SIZES argument:' + str(
                N_GRAM_SIZES) + ", please review documentation for proper format (e.g., [1])")
        ...
        if (self.DO_BOW == False and self.DO_WORD2VEC == False and self.DO_PARAGRAPH_VECTOR == False): raise ValueError(
            "DO_BOW and DO_WORD2VEC and DO_PARAGRAPH_VECTOR cannot both be false")
    ...
    def build_corpus(self):
        ...
        self.df_data = join_montage_files(self.data_dir, self.NAME_UNID_REPORTS,
                                          self.NAME_TEXT_REPORTS)  # build dataframe with all the report text
        ...
        if (self.DO_WORD2VEC or self.DO_PARAGRAPH_VECTOR):
            self.train_docs, self.test_docs, self.total_train_words = build_d2v_corpora(self.df_data, self.d2v_inputs,
                                                                                        self.labeled_indices)
            self.d2vmodel = train_d2v(self.train_docs, self.test_docs, self.D2V_EPOCH, self.DIM_DOC2VEC, self.W2V_DM,
                                      self.W2V_WINDOW, self.total_train_words)
    ...
    def infer_labels(self):
        ...
        if (self.DO_BOW and self.DO_WORD2VEC and self.DO_PARAGRAPH_VECTOR): self.combined = np.hstack(
            (self.bow_matrix, self.w2v_matrix, self.pv_matrix))

        if (self.DO_BOW and self.DO_WORD2VEC and not self.DO_PARAGRAPH_VECTOR): self.combined = np.hstack(
            (self.bow_matrix, self.w2v_matrix))
        if (self.DO_BOW and not self.DO_WORD2VEC and self.DO_PARAGRAPH_VECTOR): self.combined = np.hstack(
            (self.bow_matrix, self.pv_matrix))
        if (not self.DO_BOW and self.DO_WORD2VEC and self.DO_PARAGRAPH_VECTOR): self.combined = np.hstack(
            (self.w2v_matrix, self.pv_matrix))
        ...
        if (self.DO_WORD2VEC):            self.headers = self.headers + ["W2V" + str(i) for i in
                                                                         range(0, self.DIM_DOC2VEC)]
        if (self.DO_PARAGRAPH_VECTOR):    self.headers = self.headers + ["PV" + str(i) for i in
                                                                         range(0, self.DIM_DOC2VEC)]
        ...
        self.lasso_models, self.accuracy = calc_auc(self.combined, self.eligible_outcomes_aligned,
                                                    self.all_outcomes_aligned, self.N_LABELS, pred_type, self.headers,
                                                    self.ASSIGNFOLD_USING_ROW)
